...
# Check if the conda environment 'dalai-chat-hub' exists
if conda env list | grep -q 'dalai-chat-hub'; then
    echo "Conda environment 'dalai-chat-hub' already exists."
else
    echo "Creating a conda environment called 'dalai-chat-hub'..."
    conda create -n dalai-chat-hub python=$(python3 --version | cut -d ' ' -f 2)
fi

# Check if the conda environment 'dalai-chat-hub' is active
if [[ "$(conda info --envs | grep '*' | awk '{print $1}')" != "dalai-chat-hub" ]]; then
    echo "Activating the conda environment 'dalai-chat-hub'..."
    conda activate dalai-chat-hub
else
    echo "Conda environment 'dalai-chat-hub' is already active."
fi
